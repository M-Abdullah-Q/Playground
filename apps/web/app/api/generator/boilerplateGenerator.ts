export class BoilerplateGenerator {
    ...
    generateCpp(): string {
        const inputReads = this.inputFields
            .map(field => field.type.startsWith("vector")
                ? `int size_${field.name}; cin >> size_${field.name}; ${field.type} ${field.name}(size_${field.name}); for(int i = 0; i < size_${field.name}; i++) cin >> ${field.name}[i];`
...
int main() {
    ...
    generateJava(): string {
        const inputReads = this.inputFields
            .map(field => field.type.startsWith("vector")
                ? `int size_${field.name} = sc.nextInt(); ArrayList<${this.mapCppToJava(field.type.replace("vector<", "").replace(">", ""))}> ${field.name} = new ArrayList<>(); for (int i = 0; i < size_${field.name}; i++) ${field.name}.add(sc.next${this.mapJavaScannerMethod(field.type)}());`
